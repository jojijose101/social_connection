$(document).ready(function () {
  ...
  const endpoint = protocol + window.location.host + "/ws/chat/";
  ...
  function newMessage(data) {
    ...
    if (data["is_sender"] === true) {
      message_element =
        ...
        '                <div class="msg-info-time time" data-time="' +
        data["timestamp"] +
        '"></div>\n' +
        "            </div>\n" +
        "\n" +
  ...
  function updateUserToFront(data) {
    ...
    if (userData.length === 0) {
      userData =
        ...
        '            <div class="fullname text-color">' +
        userdata["fullname"] +
        "</div>\n" +
        '            <div class="username text-color">@' +
        userdata["username"] +
        "</div>" +
        "        </div>\n" +
        '        <span class="badge accent-one-bg ' +
        (senderUsername !== ownUsername ? "showing-badge" : "") +
        '" id="badge-' +
        userdata["username"] +
        '"></span>\n' +
        "    </div>\n" +
